# -*- coding: utf-8 -*-
"""
Created on Thu Jun 14 20:47:19 2018
...
import mongodb
import websocket    
import urllib, hashlib,struct,time       #2.5版本后，，，sha 已经改为hashlib使用
...
class ZB_Sub_Spot_Api(object):
    ...
    def __init__(self):
        ...
        self.apiKey = ''        # 用户名
        self.secretKey = ''     # 密码
        self.mongodb_q=mongodb.Monogo()
        self.ws_sub_spot = None          # websocket应用对象  现货对象
        self.open_status = 0   #判断是否打开
        self.close_status = 0   #判断是否关闭
    ...
    def connect_Subpot(self, apiKey , secretKey , trace = False):
        ...
        websocket.enableTrace(trace)      #打开跟踪，查看日志
...
#        self.ws_sub_spot.run_forever()
    ...
    #----------------------------------------------------------------------
    def onMessage(self, ws, evt):#这个对象在接收到服务器返回的消息时调用。有两个参数，一个是该类本身，
                                 #一个是我们从服务器获取的字符串（utf-8格式）
        """信息推送""" 
        print(evt)
        self.mongodb_q.m_add(evt)
...
#        self.reconnect()
    ...
    def subscribeSpotDepth(self, symbol_pair):
        ...
        self.ws_sub_spot.send(self.ws_sub_spot,req)
